...
public abstract class Qry {
	...
	protected boolean docIteratorHasMatchUnrankedAdd(RetrievalModel r) {
		int minDocid = Qry.INVALID_DOCID;
		for (int i = 0; i < this.args.size(); i ++) {
			...
			if (q_i.docIteratorHasMatch (r)) {
				...
				if (i == this.args.size() - 1) {
					if (set.size() == 1) {
						...
						docIteratorSetMatchCache (ite.next());
						set.clear();
						return true;
					}
					else {
						...
						Iterator<Integer> ite = set.iterator();
						if (ite.hasNext()) {
							max = (ite.next() > max) ? ite.next() : max;
							docIteratorAdvanceTo(max);
							set.clear();
							docIteratorHasMatchUnrankedAdd(r);
						}
					}
				}
			}
